...
/**
    ...
    fun getImage( path: String?): Bitmap? {
        ...
        try {
            ...
            val matrix = Matrix()
            val rotation: Float = rotationForImage( Uri.fromFile(File(path)))
            if (rotation != 0f) {
    ...
    fun rotationForImage( uri: Uri): Float {
        try {
    ...
    fun getCroppedData(): CroppedBitmapData {
        ...
        if (bitmapRect.intersect(croppedBitmapRect).not()) {
            return CroppedBitmapData(croppedBitmap = bitmap ,getImagePath(context, bitmap))
        }
        ...
        bitmap?.let {
            ...
            mainHandler.post {
                for (listener in listeners) {
                    ...
                    listener.onSuccess(croppedBitmap,getImagePath(context, croppedBitmap))
                }
            }

            return CroppedBitmapData(croppedBitmap = croppedBitmap ,getImagePath(context, croppedBitmap))
        }
    ...
    companion object {
        ...
        fun getImagePath(inContext: Context, bitmap: Bitmap?): String {
            val baseDirectory: File = createGalleryAlbumDirectory(inContext)

            if (!baseDirectory.exists()) {
                baseDirectory.mkdirs()
            }
            Log.e("TAG", "saveBitmapIntoSdcard baseDirectory : $baseDirectory")
            try {
                val timeStamp: String =
                    java.text.SimpleDateFormat("yyyy_MM_dd_HH_mm_ss.SSSSSS").format(java.util.Date())
                val filename = "$timeStamp.jpg"
                var out: OutputStream? = null
                val file: File = File(baseDirectory, "/$filename")
        ...
        fun createGalleryAlbumDirectory(context: Context): File {
            val folder: File =
                File(context.getExternalFilesDir(android.os.Environment.DIRECTORY_DCIM).toString())
            folder.mkdirs()
            return folder
        }
